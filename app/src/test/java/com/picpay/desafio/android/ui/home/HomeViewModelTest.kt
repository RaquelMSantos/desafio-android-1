...
class HomeViewModelTest {
    ...
    private val observerLiveData : Observer<LiveDataState> = mock()
    ...
    fun whenStartViewModelWithSuccess_shouldReturnListOfUsers() = testCoroutineDispatcher.runBlockingTest {
        ...
        verify(observerLiveData).onChanged(LiveDataState.success(mockedList()))
    }
    ...
    fun whenStartViewModelWithError_shouldReturnErrorMessage() = testCoroutineDispatcher.runBlockingTest {
        ...
        verify(observerLiveData).onChanged(value.error?.let { LiveDataState.error(it) })
    }
